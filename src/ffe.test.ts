...
const ficheClosed = readFileSync(new URL("./fixtures/ffe_fiche_closed.html", import.meta.url), "utf8");
const resultats = readFileSync(new URL("./fixtures/ffe_resultats.html", import.meta.url), "utf8");

globalThis.fetch = (async (url: string) =>
  new Response(
    url.includes("Ref=72157") ? ficheClosed : url.includes("FicheTournoi") ? fiche : resultats,
  )) as typeof fetch;

const info = await fetchFiche("https://www.echecs.asso.fr/FicheTournoi.aspx?Ref=69309");
...
assert.equal(info.resultsLinks.Ga, "https://www.echecs.asso.fr/Resultats.aspx?URL=Tournois/Id/69309/69309&Action=Ga");

const closedInfo = await fetchFiche("https://www.echecs.asso.fr/FicheTournoi.aspx?Ref=72157");
assert.equal(closedInfo.numRounds, 5);
assert.equal(closedInfo.cadenceText, "60' + [30'']");
assert.equal(closedInfo.resultsLinks.Ga, undefined, "closed/round-robin tournament has no Ga link");
assert.ok(closedInfo.resultsLinks.Berger, "expected a Grille Berger link instead");
