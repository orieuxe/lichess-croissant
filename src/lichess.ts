...
export async function listStudies(username: string): Promise<StudyRef[]> {
  const res = await fetch(`${API}/study/by/${username}`, { headers: authHeaders() });
...
export async function downloadStudy(studyId: string): Promise<string> {
  const res = await fetch(`${API}/study/${studyId}.pgn`, { headers: authHeaders() });
...
export function studiesNotDownloaded(studies: StudyRef[], manifest: Record<string, string>): StudyRef[] {
